...
function buildSearchQuery(params, isCountQuery = false, options = {}) {
  ...
  if (cardQueries.length > 0) {
    ...
    if (langs.length > 0) {
        cardQueries.forEach((query, i) => {
            ...
            const allConditions = [...langConditions, `C_${alias}.desc LIKE ?`].join(' OR ');
  ...
  if (params.has('after_date')) { conditions.push("D.upload_date >= ?"); bindings.push(Date.parse(params.get('after_date'))); }
  if (params.has('before_date')) { conditions.push("D.upload_date <= ?"); bindings.push(Date.parse(params.get('before_date'))); }
...
export default async function handleSearchRequest(request, env) {
  ...
  if (!dataResult.success || !countResult.success) {
    console.error("数据库批量查询失败。", { dataResult, countResult });
